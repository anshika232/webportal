...
$sql1 = "select * from students where reg_no = '$_SESSION[reg_no]'";
    ...
    $status = $r2['status'];
    if($status < 5){
      header('location:resulterror.php');
    }
}
?>
...
$c1 = mysqli_num_rows($q1);
    ...
    $sql2 = "select sem from semester where prgm = '$prgm' order by sem desc limit 1";
    $q2 = mysqli_query($con , $sql2);
    $r2 = mysqli_fetch_assoc($q2);
    $sem = $r2['sem'];
...
<table class="tablecard table table-striped table-hover" id = "table">
  <thead>
    <tr>
...
<ul class="nav nav-tabs " id="myTab" role="tablist">
  ...
  <li class="nav-item">
    <a class="nav-link active" id="messages-tab" data-toggle="tab" href="result.php" role="tab" aria-controls="messages" aria-selected="false">Result</a>
  </li>
  <li class="nav-item">
...
<p> Reg no. : <?php echo $r1['reg_no']?></p>
<p> Programme. : <?php echo $r1['programme']?></p>
